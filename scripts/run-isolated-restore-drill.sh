...
AS $$
  SELECT jsonb_build_object(
    ...
    'tables', jsonb_build_object(
      ...
      'security_events', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.security_events row),
      'scheduled_job_runs', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.scheduled_job_runs row),
      ...
      'communication_preferences', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.customer_id, row.channel, row.purpose), '[]'::JSONB) FROM public.communication_preferences row),
      'data_subject_requests', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.data_subject_requests row),
      'privacy_action_queue', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.privacy_action_queue row),
      'privacy_action_approvals', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.privacy_action_approvals row),
      'privacy_audit_log', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.privacy_audit_log row),
      'leads', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.leads row),
      'lead_events', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.lead_events row),
      ...
      'appointment_events', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.appointment_events row),
      ...
      'message_outbox', (SELECT COALESCE(jsonb_agg(to_jsonb(row) ORDER BY row.id), '[]'::JSONB) FROM public.message_outbox row),
