...
export default function StatusTimeline({
  ...
  const progress = Math.max(0, Math.min(100, (currentIndex / (mainSteps.length - 1)) * 100));
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative pt-4 pb-10">
        ...
        {/* Active progress */}
        <div
          className="absolute top-[34px] left-[5%] h-1 bg-blue-500 rounded-full z-0 transition-all duration-700 shadow-[0_0_12px_rgba(59,130,246,0.5)]"
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 border-2 ${
                  isDone    ? "bg-blue-600 border-blue-600 shadow-[0_0_12px_rgba(59,130,246,0.5)]" :
                  isCurrent ? "bg-blue-500 border-blue-400 ring-4 ring-blue-500/20 shadow-[0_0_16px_rgba(59,130,246,0.6)]" :
                              "bg-white dark:bg-[#0d1420] border-slate-300 dark:border-[#2d3f55] group-hover:border-blue-500/60"
                }`}>
                  {isDone    ? <Check className="w-3.5 h-3.5 text-slate-900 dark:text-white" /> :
                   isCurrent ? <Edit2 className="w-3 h-3 text-slate-900 dark:text-white" /> :
                               <Clock className="w-3 h-3 text-slate-600 group-hover:text-blue-400" />}
                </div>
                <div className="text-center">
                  <p className={`text-[10px] font-semibold leading-tight whitespace-pre-line text-center ${
                    isCurrent ? "text-slate-900 dark:text-white" : isDone ? "text-blue-400" : "text-slate-500"
                  }`}>{step.label}</p>
                  <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full mt-0.5 inline-block ${
                    ...
                    isCurrent ? "bg-blue-500/10 text-blue-400" :
                                "bg-slate-200 dark:bg-[#1f2937] text-slate-600"
                  }`}>
                    {isDone ? "Tamamlandı" : isCurrent ? "Aktif" : "Bekliyor"}
                  </span>
                </div>
              </button>
            );
